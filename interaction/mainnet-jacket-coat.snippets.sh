...
SC_ADDRESS=$(mxpy data load --key=address-mainnet-jacket-coat)
CHAIN=1
SCRIPT_DIR=$(dirname $0)
FIRST_TOKEN_ID=JACKET-c44ffb
SECOND_TOKEN_ID=COAT-5612d2
...
deploy() {
    ...
    mxpy data store --key=address-mainnet-jacket-coat --value=${SC_ADDRESS}
...
verify() {
    mxpy contract verify "${SC_ADDRESS}" \
        ...
        --docker-image="multiversx/sdk-rust-contract-builder:v5.3.0" \
        --keyfile=${1}
}
